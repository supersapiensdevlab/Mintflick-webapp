...
function sanitizeFilename(file: any) {
  ...
  const renamedFile = new File([file], 'video', { type: file.type });
  return renamedFile;
}
...
export default function VideoInput(props: Props) {
  ...
  async function onVideoFileChange(event: any) {
    event?.target.files[0]
      ? props.setSelectedVideo(sanitizeFilename(event.target.files[0]))
  ...
  return (
    <div className='flex flex-col items-center justify-center w-full gap-2 p-2 border-2 border-dashed rounded-lg cursor-pointer border-vapormintBlack-200 text-brand4'>
      ...
      {props.selectedVideo && (
        <div className='object-cover w-full overflow-hidden rounded-lg aspect-video bg-vapormintBlack-200'>
          <ReactPlayer
            ...
            muted={true}
            volume={0.5}
            url={URL.createObjectURL(props.selectedVideo)}
            controls={true}
          />
        </div>
      )}
      <label className='flex items-center justify-start w-full gap-2 p-1 text-base font-semibold capitalize cursor-pointer text-vapormintWhite-100'>
        ...
        {props.selectedVideo
          ? props.selectedVideo.name.substring(0, 16)
          : props.label}
      </label>
    </div>
  );
