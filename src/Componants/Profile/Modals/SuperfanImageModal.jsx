...
const ProfileImageModal = ({ setShowPlanImageModal }) => {
  const State = useContext(UserContext);
  const [selectedProfileImage, setSelectedPlanImage] = useState(null);
  ...
  const handleUpdateProfileImage = () => {
    if (selectedProfileImage && selectedProfileImage.file[0]) {
      ...
      uploadFile(selectedProfileImage?.file[0]).then(async (cid) => {
        ...
        axios
          .post(
            `${process.env.REACT_APP_SERVER_URL}/user/update_superfanplans`,
            formData,
            {
              headers: {
