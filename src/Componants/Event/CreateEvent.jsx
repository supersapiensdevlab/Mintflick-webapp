...
function CreateEvent() {
  ...
  const handleStartDateChange = (event) => {
    ...
    setendDate((prevEndDate) => {
      if (prevEndDate === '' || new Date(prevEndDate) > new Date(event.target.value)) {
        return prevEndDate;
      }
...
const _endDate = new Date(endDate);
  ...
  if(!State.database.provider)alert("You need to connect to a web3 wallet to use this feature!") 
  ...
  const params = lockInterface.encodeFunctionData(
    ...
    [
      Address,
      diffInDays * 60 * 60 * 24, // 30 days in seconds
