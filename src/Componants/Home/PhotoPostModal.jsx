...
function PhotoPostModal({ setphotoPostModalOpen }) {
    ...
    const handleSubmit = (e) => {
        ...
        uploadFile(selectedPost.file).then(async (cid) => {
            ...
            if (isNFT) {
                ...
                let metadata = {
                    image: 'https://ipfs.io/ipfs/' + cid + '/' + selectedPost.file[0].name,
                    external_url: 'https://ipfs.io/ipfs/' + cid + '/' + selectedPost.file[0].name,
                    ...
                    animation_url: 'https://ipfs.io/ipfs/' + cid + '/' + selectedPost.file[0].name,
                };
                const blob = new Blob([JSON.stringify(metadata)], { type: 'application/json' });
                const metaFile = [new File([blob], 'meta.json')];
                const meta_cid = await uploadFile(metaFile);
        ...
        <div className="modal-box p-0 bg-slate-100 dark:bg-slate-800 ">
            ...
            <form onSubmit={handleSubmit}>
                <div className="w-full p-4 space-y-3">
                    ...
                    <label
                        htmlFor="post_announcement_image" className=" cursor-pointer flex items-center gap-2  w-full p-2 border-2 border-slate-400 dark:border-slate-600 border-dashed rounded-lg text-brand4">
                        <File />  {selectedPost ? (
                            selectedPost.file ? (
                    ...
                    {selectedPost ? (
                            selectedPost.file ? (
                                <div className='max-h-36 flex justify-center'><div><img className='max-h-36' src={selectedPost.localurl} ></img></div></div>
                            ) : null
                        ) :<></>}
                    <textarea
                    ...
                    <div className="w-fit flex space-x-2">
                        <label className="flex items-center cursor-pointer gap-2">
                            <input type="checkbox" value={isNFT} onChange={() => setIsNFT(!isNFT)} className="checkbox checkbox-primary" />
                            <span className="label-text text-brand3">Mint as NFT</span>
                        </label>
                        {isNFT && <div className="form-control">
                    ...
                    <button type={'submit'} className="btn btn-brand w-full">Post</button>
