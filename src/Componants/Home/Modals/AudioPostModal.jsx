  ...
  const handleSubmit = (e) => {
    ...
    if (!isNFT && !uploadingTrack) {
      ...
      storeWithProgress(files).then((cid) => {
        ...
        formData.append('trackFile', selectedTrack.file, selectedTrack.file.name);
        formData.append('trackImage', selectedThumbnail.file, selectedThumbnail.file.name);
        ...
        <div className='w-full p-4 space-y-3'>
          <div className='flex flex-col sm:flex-row gap-1'>
            <div
              ...
              <label htmlFor="trackthumbnail" className='flex gap-1 cursor-pointer'>
                <input
                  id="trackthumbnail"
                  type="file"
                  name="trackImage"
                ...
                {selectedThumbnail && selectedThumbnail.file ? selectedThumbnail.file.name.substring(0, 16) : 'Choose album image'}
              </label>
            </div>
            <div
              htmlFor=''
              ...
              <label htmlFor="trackfile" className='flex gap-1 cursor-pointer'>
                <input
                  ...
                  className="sr-only "
                />
                {selectedTrack && selectedTrack.file ? <FileCheck className="text-emerald-700" /> : <FileMusic />}
                {selectedTrack && selectedTrack.file ? selectedTrack.file.name.substring(0, 16) : 'Choose audio file'}
              </label>
            </div>
          </div>
          <input
          ...
          <div className='flex space-x-2'>
            <select className='select flex-grow' onChange={(e) => setTrack({ ...track, genre: e.target.value })}>
              <option disabled selected>
                Genre
              </option>
              {genre.map((c) => <option>{c}</option>)}
            </select>
            <select className='select flex-grow' onChange={(e) => setTrack({ ...track, mood: e.target.value })}>
              <option disabled selected>
                Mood
              </option>
              {mood.map((c) => <option>{c}</option>)}
            </select>
          </div>
          ...
          <textarea
            ...
            value={track.description}></textarea>
          <span
          ...
          {advancedOptionsShow && (
            <>
              ...
              <div className='flex gap-1 w-full flex-wrap'>
                <select className='select select-xs flex-grow' onChange={(e) => setTrack({ ...track, allowAttribution: e.target.value })}>
                  <option disabled selected>
                    Allow Attribution?
                  </option>
                  {attribution.map((c) => <option>{c}</option>)}
                </select>
                <select className='select select-xs flex-grow' onChange={(e) => setTrack({ ...track, commercialUse: e.target.value })}>
                  <option disabled selected>
                    Commercial Use?
                  </option>
                  {commercialUse.map((c) => <option>{c}</option>)}
                </select>
                <select className='select select-xs flex-grow' onChange={(e) => setTrack({ ...track, derivativeWorks: e.target.value })}>
                  <option disabled selected>
                    Derivative Works?
                  </option>
          ...
          <button
            type={"submit"}
            className={`btn  w-full ${uploadingTrack ? "loading" : "btn-brand"}`}
          >
            Post audio
          </button>
        </div>
