...
function ShowComment({ comment, user_id, contentData, setCommentCount, replyTo, original }) {
  ...
  useEffect(() => {
    ...
    if (replyTo) {
      axios.get(
        ...
        setProfile(res.data ? res.data.profile_image : 'https://cdn-icons-png.flaticon.com/512/149/149071.png')
    ...
    >
      ...
      <div className="flex flex-col flex-grow">
        <p className=" text-brand4 text-base font-medium">
          <span className="text-brand3 font-semibold mr-1">{replyTo ? name : comment.name}</span>
          {comment.comment}
        </p>
        ...
        {isReply && <div className="flex gap-2 items-center">
          <textarea
          ...
          <button
            ...
            className={`btn    ${replyText !== "" ? "btn-primary btn-outline" : "btn-disabled"
              }`}
          >
            <ArrowNarrowRight />
          </button>
