...
function Addcomment({user_id, contentData,myComments, setMyComments}) {
  ...
  const handleKeyPress = async(event) => {
    if(event.key === 'Enter' && user && comment !== ''){
      ...
      setMyComments((myComments) => [...myComments, comment]);
      setComment('');
    }
  }
  return (
    user && (
      <div className="relative flex items-center mx-4 my-1">
        ...
        <div className='flex items-center flex-grow border-2 border-gray-700 rounded-2xl'>
          <input value={comment} onKeyPress={handleKeyPress} onChange={(e)=>{setComment(e.target.value)}} placeholder='Add a comment' type="text" className='w-full focus:border-opacity-0 placeholder-dbeats-light text-dbeats-light text-sm h-8 border-none bg-transparent'></input>
